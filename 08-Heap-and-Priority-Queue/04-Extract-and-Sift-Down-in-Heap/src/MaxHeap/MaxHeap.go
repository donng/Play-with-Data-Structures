...
import (
	"Play-with-Data-Structures/08-Heap-and-Priority-Queue/04-Extract-and-Sift-Down-in-Heap/src/Array"
...
func (h *MaxHeap) siftDown(k int) {
	for leftChild(k) < h.data.GetSize() {
		...
		if j+1 < h.data.GetSize() && Interfaces.Compare(h.data.Get(j+1), h.data.Get(j)) > 0 {
			j++
		}
		// data[j] 是 leftChild 和 rightChild 中的最大值
		if Interfaces.Compare(h.data.Get(k), h.data.Get(j)) > 0 {
			break
		}
