...
func (at *AVLTree) IsBST() bool {
	...
	for i := 1; i < len(keys); i++ {
		if Interfaces.Compare(keys[i-1], keys[i]) > 0 {
			return false
		}
	}
	return true
...
func (at *AVLTree) getNode(n *Node, key interface{}) *Node {
	// 未找到等于 key 的节点
	if n == nil {
		return nil
	}
	...
	if Interfaces.Compare(key, n.key) == 0 {
		...
		return at.getNode(n.left, key)
	} else {
		return at.getNode(n.right, key)
	}
}
...
func (at *AVLTree) Add(key interface{}, value interface{}) {
	at.root = at.add(at.root, key, value)
}

// 向以node为根的二分搜索树中插入元素(key, value)，递归算法
// 返回插入新节点后二分搜索树的根
func (at *AVLTree) add(n *Node, key interface{}, value interface{}) *Node {
	if n == nil {
		at.size++
	...
	if Interfaces.Compare(key, n.key) < 0 {
		n.left = at.add(n.left, key, value)
	} else if Interfaces.Compare(key, n.key) > 0 {
		n.right = at.add(n.right, key, value)
	} else {
		n.value = value
	}
